<?php
namespace PaymentBundle\Entity\Manager;

use CoreBundle\Entity\Manager\AbstractCommonManager;
use CoreBundle\Repository\AbstractCommonRepository;

use PaymentBundle\Entity\Manager\Interfaces\ProductManagerInterface;
use PaymentBundle\Entity\Product;

use Symfony\Component\Form\FormFactoryInterface;
use Symfony\Component\Form\FormTypeInterface;
use Symfony\Component\Routing\RouterInterface;


class ProductManager extends AbstractCommonManager implements ProductManagerInterface
{
    /**
     * @var FormTypeInterface
     */
    protected $searchFormType;

    /**
     * @var FormFactoryInterface
     */
    protected $formFactory;

    /**
     * @var RouterInterface $router
     */
    protected $router;

    /**
     * @inheritdoc
     */
    public function getProductCotisation()
    {
        return $this->repository->getProductCotisation();
    }

    /**
     * @inheritdoc
     */
    public function __construct(AbstractCommonRepository $repository)
    {
        parent::__construct($repository);
    }

    /**
     * @inheritdoc
     */
    public function getResultFilterPaginated($requestVal, $limit = 20, $offset = 0)
    {
        return $this->repository->getResultFilterPaginated($requestVal, $limit, $offset);
    }

    /**
     * @inheritdoc
     */
    public function getResultFilterCount($requestVal)
    {
        return $this->repository->getResultFilterCount($requestVal);
    }

    /**
     * @inheritdoc
     */
    public function getProductSearchForm(Product $product)
    {
        return $this->formFactory->create(
            $this->searchFormType,
            $product,
            [
                'action' => $this->router->generate('product_list'),
                'method' => 'GET',
            ]
        );
    }

    /**
     * @inheritdoc
     */
    public function setState(Product $product)
    {
        if($product->getEnabled() == 0){
            $product->setEnabled(1);
            $product->setPublishedAt(new \DateTime());
        } else {
            $product->setEnabled(0);
            $product->setUpdatedAt(new \DateTime());
        }
        $this->save($product, false, true);
    }

    /**
     * @inheritdoc
     */
    public function setSearchFormType($searchFormType)
    {
        $this->searchFormType = $searchFormType;
        return $this;
    }

    /**
     * @inheritdoc
     */
    public function setFormFactory($formFactory)
    {
        $this->formFactory = $formFactory;
        return $this;
    }

    /**
     * @inheritdoc
     */
    public function setRouter($router)
    {
        $this->router = $router;
        return $this;
    }

    /**
     * @inheritdoc
     */
    public function getLabel()
    {
        return 'productManager';
    }
}
