...
public class SimplexeIteration {
	...
	public String toString() {
		...
		if(!this.result.isEmpty()){		
            ...
            valeur+=" \nDictionnaire \n";
            valeur+="Z "+result.getValueAt(0, result.getColumnCount()-1);
            
            for(j=1;j<result.getColumnCount()-1;j++){
                if(result.getValueAt(0,j)==0) continue ;
                valeur+="      "+result.getElementAt(0,j).getName()+" ";
            }
            valeur+="\n";
            
            
            for(i=1;i<result.getRowCount();i++){
                valeur+= result.getElementAt(i, result.getColumnCount()-1).getName()+" "+result.getValueAt(i,result.getColumnCount()-1)+" ";
                
                for(j=2;j<result.getColumnCount()-1;j++){
                    if(result.getValueAt(0,j)==0) continue ;
                     valeur+=" "+result.getValueAt(i,j) +" ";
                }
                valeur+="\n";
            }
